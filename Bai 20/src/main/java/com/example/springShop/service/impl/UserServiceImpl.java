...
import com.example.springShop.repository.RoleRepository;
...
public class UserServiceImpl implements UserService {
    ...
    private void applyRoleFromDto(User user, String roleName){
       String normailzed = (roleName == null || roleName.isBlank()) ?"USER" : roleName.trim().toUpperCase();

        Role role = roleRepository.findByName(normailzed)
                .orElseGet(()->{
                    Role r = new Role();
                    r.setName(normailzed);
                    r.setDeleted((byte) 0);
