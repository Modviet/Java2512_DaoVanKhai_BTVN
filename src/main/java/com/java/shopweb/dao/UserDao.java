...
public class UserDao extends BaseDao<UserDTO> {
    ...
    public UserDTO findByEmailAndPassword(String email,String passwordHash){
        ...
        List<UserDTO> userDTOS = query(sql,rs->mapRow(rs),email,passwordHash);
    ...
    public UserDTO findByRememberToken(String token){
        String sql = "SELECT * FROM users WHERE remember_token = ? LIMTI 1";
    ...
    public void updateRemembetToken(int userId,String token){
